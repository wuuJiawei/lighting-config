...
- Dashboard 额外轮询 `/lighting-config/api/admin/console/cache-miss`，将服务端缓存穿透事件可视化，方便回归排障。
...
## 下一步建议
- 与 Server Agent 对齐 `/api/admin/config`、`/api/namespace`、`/api/audit` 的最终 DTO，并在 `src/api` 中替换 mock。
